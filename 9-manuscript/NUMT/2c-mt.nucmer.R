...
######################
######################
# file to store NUMT regions
######################
numt_region_out <- "2o-A188.NUMT.regions.txt"
cat("chr\tstart\tend\n", file = numt_region_out)

######################
# plot
######################
#pdf("chrEnd.features.B71Ref2.pdf", width = 5, height = 5)
par(mar = c(1, 2, 3, 2))

xlabel <- ""
ylabel <- "Physical positoin (Mb)"
mainlabel <- "NUMTs on A188Ref1"
min_ref_match <- 5000
min_qry_match <- 5000
min_identity <- 95
feature_closeup <- T
...
two_cols <- c("indianred3", "indianred2")
line_adjust <- 0.1
...
# nucmer file
nuc_file <- "/bulk/liu3zhen/research/A188Ref1/13-NUMT/1-nucmer/A188mt_Ref1.filt.txt"
...
pdf("2o-NUMTs_A188Ref1.pdf", width = 8, height = 5)
    ...
    for (j in 1:nrow(echr_aln)) {
      ...
      if (feature_closeup) {
        ...
        if ((y_start - previous_pos) > 100000 & previous_pos != 0) {
          ...
          # region output
          cat(echr, closeup_new_pos, previous_pos, sep = "\t", file = numt_region_out, append = T)
          cat("\n", file = numt_region_out, append = T)
